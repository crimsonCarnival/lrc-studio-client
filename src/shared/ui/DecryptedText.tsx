...
interface DecryptedTextProps extends HTMLMotionProps<'span'> {
  ...
  // 'change' mode renders plain on mount by default (only re-animates on
  // subsequent text changes, e.g. a language switch). Set true to also
  // decrypt-animate the initial mount — e.g. an error page's headline.
...
export default function DecryptedText({
  ...
  useEffect(() => {
    ...
    // 'change': decrypt-animate whenever the text prop changes after mount
    // (e.g. i18n language switch swaps every t() string). Mount renders plain,
    // unless animateOnMount opts into animating the first render too.
    if (animateOn === 'change') {
      const isFirstRender = isFirstRenderRef.current;
      isFirstRenderRef.current = false;
      const textChanged = prevTextRef.current !== text;
      prevTextRef.current = text;
      const shouldAnimate = textChanged || (isFirstRender && animateOnMount);
    ...
    setRevealedIndices(new Set());
    setDirection('forward');
  }, [animateOn, text, shuffleText, triggerDecrypt, animateOnMount]);
