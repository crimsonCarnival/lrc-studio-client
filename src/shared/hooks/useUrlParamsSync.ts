...
export function useUrlParamsSync(appState: UrlAppState, layoutState: UrlLayoutState) {
  ...
  useEffect(() => {
    ...
    const updateParam = (key: string, val: string | number | null, defaultVal: string | number | null, force = false) => {
    ...
    // Sync Speed
    updateParam('speed', playbackSpeed === 1 ? null : playbackSpeed.toFixed(2), null);
