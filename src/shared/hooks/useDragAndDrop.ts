...
interface DragDropPlayerHandle {
...
interface DragAndDropOptions {
  ...
  requestConfirm: (message: string, action: () => void, options?: { title?: string; variant?: string }) => void;
  playerRef: React.RefObject<DragDropPlayerHandle | null>;
}
...
export function useDragAndDrop({
      ...
      if (['lrc', 'srt', 'txt'].includes(extension)) {
        ...
        try {
          ...
          const applyImport = () => {
            ...
            toast.success(
              (t as unknown as (k: string, o?: Record<string, unknown>) => string)('import.success', { count: parsedLines.length }) ||
  ...
  useEffect(() => {
    const onDragEnter = (e: DragEvent) => handleDragEnterRef.current(e);
    const onDragLeave = (e: DragEvent) => handleDragLeaveRef.current(e);
    const onDragOver = (e: DragEvent) => handleDragOverRef.current(e);
    const onDrop = (e: DragEvent) => handleDropRef.current(e);
