...
export function BadgeChip({ id, className = '' }: { id: string; className?: string }) {
  const { t, i18n } = useTranslation();
  const defs = useBadgeDefs();
  const dbDef = defs[id];
  const staticDef = BADGE_REGISTRY[id];
  const def = dbDef || staticDef;
  if (!def) return null;
  ...
  return (
    <Tip content={tip || undefined} side="bottom">
      ...
      >
        {isShimmer ? (
          <span className="badge-shimmer-txt">{t(`badges.${id}.label`, { defaultValue: labelText })}</span>
        ) : (
          <span>{t(`badges.${id}.label`, { defaultValue: labelText })}</span>
        )}
      </span>
    </Tip>
  );
