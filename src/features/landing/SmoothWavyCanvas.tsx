...
const SmoothWavyCanvas = ({ animationSpeed = 0.004, lineOpacity = 1 }: SmoothWavyCanvasProps) => {
  ...
  const frameTimesRef = useRef<number[]>([]);      // rolling window of last 10 frame durations (ms)
  ...
  const animate = useCallback((timestamp: number) => {
    ...
    const { bg, primary, secondary, accent, opacityScale } = colors;
