...
interface PanelContentProps {
  ...
  validateShortcut: (newKey: string, currentKeyName: string) => boolean;
  isGuest: boolean;
  userId?: string;
...
export default function SettingsPage() {
  ...
  // navigate accepts a path string or a numeric delta; loosen for union/dynamic targets.
  const navAny = navigate as (to: string | number) => void;
  const location = useLocation();
  const { t } = useTranslation();
  // Tab labels & group labels use dynamic keys.
  ...
  const [settings, setSettings] = useState<Settings>(globalSettings || DEFAULT_SETTINGS);
  const [searchTerm, setSearchTerm] = useState('');
  const [hasResults, setHasResults] = useState(true);
  const panelRef = useRef<HTMLDivElement>(null);
  const autoSaveRef = useRef<Settings | null>(null);
  const savedSettingsRef = useRef<Settings>(globalSettings || DEFAULT_SETTINGS);
  ...
  const validateShortcut = useCallback((newKey: string, currentKeyName: string) => {
  ...
  const guardedNavigate = (target: string | number) => {
    if (isDirty && !settings.advanced?.autoSave?.enabled) {
              ...
              return (
                <div key={entry.id}>
                  ...
                  >
                    ...
                    <Icon name={entry.iconName} size={14} className={`shrink-0 ${isActive ? 'text-primary' : 'text-zinc-500'}`} />
                    {label}
                  </button>
                </div>
              );
        ...
        <div className="flex flex-col flex-1 min-h-0 min-w-0">
          ...
          <div className="flex-1 min-h-0 flex flex-col relative">
            <div className="flex-1 min-h-0 overflow-y-auto settings-scroll pb-28">
              <div ref={panelRef} className={`${activeTab === 'stats' ? 'w-full max-w-none' : 'max-w-2xl'} mx-auto px-4 sm:px-6 lg:px-8 py-6`}>
              ...
              <div className="fixed inset-0 z-popover flex items-center justify-center">
                ...
                <div className="relative z-10 bg-zinc-900 border border-zinc-800 rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl">
                  ...
                  <div className="flex flex-col gap-2">
                    <Button
                      onClick={() => { handleApply(); setShowLeaveModal(false); navAny(pendingNavRef.current!); }}
                    ...
                    <Button
                      variant="ghost"
                      onClick={() => { setShowLeaveModal(false); navAny(pendingNavRef.current!); }}
