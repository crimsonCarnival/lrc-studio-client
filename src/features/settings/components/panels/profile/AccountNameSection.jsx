...
export default function AccountNameSection() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div id="section-handle-history" className="space-y-1.5 pt-2 border-t border-border scroll-mt-4">
        <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">
          {t('profile.sections.handleHistory')}
        </p>
        {user?.previousAccountNames?.length > 0 ? (
          <ul className="space-y-1">
            {user.previousAccountNames.map((entry, i) => (
