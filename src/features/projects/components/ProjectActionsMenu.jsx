...
/** @param {{ icon?: any, label: string, onClick?: () => void, danger?: boolean, children?: React.ReactNode }} props */
function MenuItem({ icon: Icon, label, onClick, danger, children }) {
  const [open, setOpen] = useState(false);
  const style = danger ? { color: 'var(--color-red-400, #f87171)' } : undefined;
...
function AddToListMenu({ user, project, t }) {
  const [open, setOpen] = useState(false);
  /** @type {[any[] | null, React.Dispatch<React.SetStateAction<any[] | null>>]} */
  const [playlists, setPlaylists] = useState(/** @type {any[] | null} */ (null));
  ...
  const handleAdd = async (playlistId) => {
    ...
    try {
      await addProjectToPlaylist(playlistId, project.projectId);
  ...
  const handleNewPlaylist = async () => {
    try {
      ...
      if (pl?.id) {
        await addProjectToPlaylist(pl.id, project.projectId);
...
export function ProjectActionsMenu({
  ...
  const handleShareOg = async () => {
    ...
    const ogUrl = `${origin}/og/project/${project?.projectId}`;
    if (navigator.share) {
