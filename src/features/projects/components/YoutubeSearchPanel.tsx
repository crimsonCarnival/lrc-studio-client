...
interface YoutubeSearchPanelProps {
  onSelect: (selection: YtSelection) => void;
  onClose?: () => void;
  initialQuery?: string;
}

export default function YoutubeSearchPanel({ onSelect, onClose, initialQuery = '' }: YoutubeSearchPanelProps) {
  ...
  const [unembeddableWarning, setUnembeddableWarning] = useState<UnembeddableWarning | null>(null);
  ...
  const runSearch = useCallback(async (q: string) => {
    ...
    try {
      ...
      const data = await youtube.search(term) as { results?: YtResult[] };
  ...
  const handleSelect = (item: YtResult) => {
