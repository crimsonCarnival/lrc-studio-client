...
export function SpotifyButton({ onClick, t, loading = false }) {
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={loading}
      className="w-full h-11 bg-[#1DB954] hover:bg-[#1ed760] disabled:opacity-60 text-black font-semibold text-sm rounded-xl flex items-center justify-center gap-3 transition-all duration-200 shadow-md hover:shadow-lg hover:shadow-[#1DB954]/20"
    >
      {loading ? (
      ...
      {t('auth.spotifyContinue', 'Continue with Spotify')}
    </button>
  );
}
