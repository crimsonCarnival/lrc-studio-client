...
export default function PasswordStrength({ password }: { password?: string }) {
  ...
  // Keys are composed from validated maps below; bypass strict key checking.
  const tk = t as (key: string) => string;
  const [zxcvbn, setZxcvbn] = useState<ZxcvbnFn | null>(null);
  useEffect(() => { import('zxcvbn').then((m) => setZxcvbn(() => m.default as unknown as ZxcvbnFn)); }, []);
  ...
  const warning = warningKey
    ? tk(`auth.passwordWarnings.${warningKey}`)
    ...
    <div className="mt-1 space-y-1">
      ...
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs text-zinc-400">
          {tk(`auth.passwordStrength.${level.key}`)}
        </p>
        {warning && (
