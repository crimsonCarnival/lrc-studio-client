...
function normaliseIdentifier(raw: string) {
...
export default function SetAccountNameModal() {
  ...
  // zod issue messages are validation keys; bypass strict key checking.
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (normalised === user?.accountName) {
      ...
      if (!validation.success) {
        setError(tk(validation.error.issues[0].message, 'Invalid account name format.'));
        return;
      }
    }
    ...
    try {
      ...
      setError(translateAuthError(t, err, 'generic'));
    } finally {
      setLoading(false);
    }
  };
