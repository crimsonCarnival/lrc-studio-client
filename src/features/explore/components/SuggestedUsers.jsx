...
export function SuggestedUsers({ limit = 8 }) {
  const { t } = useTranslation();
  const { user: me } = useAuthContext();
  const { users, loading, error } = useSuggestedUsers(limit);
  const [followState, setFollowState] = useState({});

  const handleFollow = useCallback(async (accountName) => {
  ...
  const handleUnfollow = useCallback(async (accountName) => {
    setFollowState(s => ({ ...s, [accountName]: 'pending' }));
    try {
