...
export function useTrendingProjects(limit = 6) {
  const [projects, setProjects] = useState<Awaited<ReturnType<typeof getTrendingProjects>>['projects']>([]);
...
export function usePopularPlaylists(limit = 6) {
  const [playlists, setPlaylists] = useState<Awaited<ReturnType<typeof getPopularPlaylists>>['playlists']>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<unknown>(null);
...
export function useSuggestedUsers(limit = 8) {
  const [users, setUsers] = useState<Awaited<ReturnType<typeof getSuggestedUsers>>>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<unknown>(null);
