...
interface LineTextContentProps {
  line: TextLine;
  lineIndex: number;
  isActive: boolean;
  isSynced: boolean;
  ...
  editingReadingWordIndex: number | null;
  setEditingReadingWordIndex: (idx: number | null) => void;
  handleReadingCommit: (val: string, wi: number, direction?: number) => void;
  selection: Selection;
  setSelection: (sel: Selection) => void;
  onCharClick: (ci: number) => void;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  handleWordClick: (e: any, w: Word, wi: number) => void;
...
const LineTextContent = memo(({
  ...
  const fmtR = (r?: string | null) => {
    ...
    return r ? (rubyFmt === 'katakana' ? toKatakana(r) : toHiragana(r)) : r;
  };
                    ...
                    <ruby>
                      ...
                      <rt>
                        <ReadingInput
                          ...
                          onCommit={(val: string, direction?: number) => handleReadingCommit(val, wi, direction)}
            ...
            : (() => {
              ...
              if (innerHasSingerSplit) {
                ...
                const displayWords: Word[] = (line.words?.length ?? 0) > 0
                  ? line.words!
                  : (line.text || '').split(/(\s+)/).filter(Boolean).reduce<Word[]>((acc, tok) => {
              ...
              return segments.map((seg) => {
                ...
                return segmentChars.map((ch, ci_off) => {
                  ...
                  const handleCharClick = (e: { stopPropagation: () => void }) => {
                    e.stopPropagation();
                    onCharClick(ci);
                  };
                        ...
                        <ruby key={ci} className="text-primary font-medium">
                          ...
                          <rt>
                            <ReadingInput
                              defaultValue=""
                              onCommit={(val: string) => {
                                const charReadings: { char: string; reading?: string | null }[] = [];
                                segments.forEach((s2) => {
                                  const chars = [...s2.text];
...
LineTextContent.displayName = 'LineTextContent';
